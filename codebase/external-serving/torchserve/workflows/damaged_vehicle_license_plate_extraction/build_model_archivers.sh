...
source $TS_ROOT/script_variables.sh
...
do
  ...
  docker run --rm --name mar -v $(pwd)/model-store:/home/model-server/model-store -v $(pwd)/$TS_ROOT/models/${model_name}:${CONTAINER_MODEL_PATH} -v $HOST_HANDLER:$CONTAINER_HANDLER --entrypoint="" pytorch/torchserve:latest $CMD_CREATE_MAR
