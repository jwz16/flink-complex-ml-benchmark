...
class LicensePlateOcrHandler(BaseHandler, ABC):
  ...
  def preprocess(self, data):
    ...
    images_batch = []
    plate_rsp = data[0].get('data') or data[0].get('body')
    plate_rsp = json.loads(plate_rsp.decode())

    for plate_result in plate_rsp:
      if 'error' in plate_result:
        continue

      for img_dict in plate_result['cropped_imgs']:
