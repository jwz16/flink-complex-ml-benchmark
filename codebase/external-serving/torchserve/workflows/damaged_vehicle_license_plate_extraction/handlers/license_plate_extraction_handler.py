...
class LicensePlateExtractionHandler(BaseHandler, ABC):
  ...
  def preprocess(self, data):
    ...
    images_batch = []
    detect_rsp = json.loads(data[0].get('vehicle_damage_detection').decode())
    img_dicts = json.loads(data[0].get('pre_processing').decode())

    for detect_result, img_dict in zip(detect_rsp, img_dicts):
      if self._has_damaged_vehicles(detect_result) is True:
